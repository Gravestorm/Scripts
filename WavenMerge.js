...
const songs = [
  ...
  {name: "Stems_05_Interlude_Amakna", instruments: [{ name: "WAVEN_STEMS_INTERLUDE"}]},
  {name: "Stems_06_Interlude_Amakna", instruments: [{ name: "WAVEN_STEMS_INTERLUDE_2"}]},
  ...
  {name: "Bukowski_01_TheNightOfTheGoat_Brakmar", instruments: [{ name: "MUS_Bonta_Bukowski_MAIN_01"}]},
  {name: "Bukowski_02_BlackClouds_Brakmar", instruments: [{ name: "MUS_Bonta_Bukowski_MAIN_02"}]},
  {name: "Bukowski_03_AboutYou_Brakmar", instruments: [{ name: "MUS_Bonta_Bukowski_MAIN_03"}]},
  {name: "Tofu_01_Flight_TofuIsland", instruments: [{ name: "MUS_Fight_Tofu_D_01"}]}
]
songs.forEach((song) => {
  const { name: name, instruments: instruments } = song
  if (instruments.length === 1) { fs.copyFile(path.join(inputDir, `${instruments[0].name}.wav`), path.join(outputDir, `${name}.wav`), err => err ? console.log(`Error copying file for ${name}: ${err}`) : console.log(`Copying for ${name} finished`)) }
  else {
    const merge = ffmpeg()
    instruments.map((instrument) => path.join(inputDir, `${instrument.name}.wav`)).forEach(file => { merge.input(file) })
    merge.complexFilter([{ filter : 'amix', options: { inputs : instruments.length, duration : 'longest', normalize: 0, dropout_transition: 0, weights: instruments.map((instrument) => instrument.weight).join(' ') }}])
    .on('end', () => { console.log(`Merging for ${name} finished`) })
    .on('error', (err) => { console.error(`Error merging for ${name}: ${err}`) })
