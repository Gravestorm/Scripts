...
const inputFile = 'discord.txt';
const urls = fs.readFileSync(inputFile, 'utf-8').split('\n');

urls.forEach((url, index) => {
  if (!url || !url.match(/\.(mp4|webm)$/i)) { // /\.(jpeg|jpg|png)$/i)  /\.(mp4|webm)$/i)  /\.(gif)$/i)
  ...
  setTimeout(() => {
    https.get(url, (response) => {
      ...
      while (fs.existsSync(outputPath)) {
        ...
        outputPath = path.join('C:\\Media\\Downloaded', `${nameWithoutExtension}_${count}${extension}`);
        count++;
      }
